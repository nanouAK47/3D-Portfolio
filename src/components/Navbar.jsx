    ...
    >
      <div className="w-full flex justify-between items-center max-w-7x1 mx-auto">
        ...
        > 
          ...
          <p className="text-white text-[18px] font-bold cursor-pointer flex">Nael &nbsp; <span className="sm:block hidden"> | Développeur web</span></p>
        </Link>
        <ul className="list-none hidden sm:flex flex-row gap-10">
          {navLinks.map((link) => (
          <li key={link.id} className={`${active === link.title ? "text-white" : "text-secondary"} hover:texte-white text-[18px] font-medium cursor-pointer`}
